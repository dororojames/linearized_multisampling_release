...
class GradientVisualizer(object):
    def __init__(self, opt):
        self.opt = opt
        self.warper = sampler.DifferentiableImageSampler(
            'bilinear', 'zeros')
    ...
    def get_next_translation_vec(self, data_pack, image_warper):
        translation_vec = data_pack['translation_vec'].clone(
        ...
        ident_mat = torch.eye(2, 3).unsqueeze(0)
        down_sampled_orig_image = self.warper.warp_image(
            orig_image, ident_mat, self.opt.out_shape).detach()
        warped_image = image_warper.warp_image(
            orig_image, translation_mat, self.opt.out_shape)
